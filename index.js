import{m as k,c as P,g as N,s as A,a as C,f as R,i as B,b as D}from"./assets/modal-controller-C1hzPEXd.js";import"./assets/vendor-BAoYjrW1.js";async function V(n){const e=await k("filters",n);if(!e)throw new Error("Cant' get filters");return e}function j(n){return n.map(e=>`
  ...
  `).join("")}function I(n,e,g,o,c,u){const i=P(e),y=o.toLowerCase()==="cardio"?"running":"fluent-food",f="/sport_energy/img/sprite.svg";return`
...
</li>`}function S(n){const e=document.querySelector(n);e==null||e.scrollIntoView({behavior:"smooth"})}function O(){var L;const n=".exercises-header",e=document.querySelector(".filters-output"),g=document.querySelector(".filter-tabs"),o=document.querySelector(".filter-pagination"),c=document.querySelector(".exercises-list"),u=document.querySelector(".exercises-pagination"),i=document.querySelector(".exercises-filters-form"),y=document.querySelector(".exercises-subheader-container");if(!e)throw new Error("Can't find .filters-output");if(!g)throw new Error("Can't find .filter-tabs");if(!o)throw new Error("Can't find .filter-pagination");if(!i)throw new Error("Can't find .exercises-filters-form");if(!y)throw new Error("Can't find .exercises-subheader-container");let f=(L=g.querySelector(".active"))==null?void 0:L.dataset.filter,l=1,x="",w,h;i.addEventListener("submit",r=>{r.preventDefault(),x=new FormData(i).get("keyword"),b(w,h,l,!1)});function v(r,t,s=!0){e.innerHTML="",o.innerHTML="",V({filter:r,page:t,limit:12}).then(a=>{e.innerHTML=j(a.results),o.innerHTML=$(a.totalPages,t),e.style.display="flex",o.style.display="flex",E(null),c.style.display="none",u.style.display="none",i.style.display="none",i.reset(),x="",M(),s&&S(n)}).catch(a=>C({title:"Error",message:a.message,position:"topRight"}))}async function b(r,t,s,d=!0){if(!c||!u)return;const a=10,T={[r]:t,page:s,limit:a,keyword:x};try{const m=await N(T),H=m.results.map(p=>I(p.rating,p.name,`${p.burnedCalories} kcal`,p.bodyPart,p.target,p._id)).join("");c.innerHTML=H,u.innerHTML=$(m.totalPages,m.page),i.style.display="flex",c.style.display="flex",u.style.display="flex",F(r,t),m.results.length===0&&A({title:"",message:" No excersies found",position:"topRight"}),d&&S(n)}catch(m){C({title:"Error",message:(m==null?void 0:m.message)||"Failed to load exercises",position:"topRight"})}}function M(){e.querySelectorAll(".filter-card").forEach(t=>{t.addEventListener("click",()=>{const s=t.getAttribute("data-filter-type"),d=t.getAttribute("data-filter-name");if(!s||!d)return;l=1;let a;if(s==="Muscles")a="muscles";else if(s==="Body parts")a="bodypart";else if(s==="Equipment")a="equipment";else return;e&&o&&(e.innerHTML="",e.style.display="none",o.innerHTML="",o.style.display="none"),E(d),w=a,h=d.toLowerCase(),b(a,h,l)})})}function E(r){y.innerHTML=r?` / <span class="exercises-subheader">${r}</span>`:""}function F(r,t){u.querySelectorAll(".pagination-page").forEach(d=>{d.addEventListener("click",()=>{const a=Number(d.getAttribute("data-page"));!a||a===l||(l=a,b(r,t,l))})})}g.addEventListener("click",r=>{const t=r.target;if(t.tagName.toLowerCase()!=="button")return;const s=t.dataset.filter;!s||s===f||(g.querySelectorAll("button").forEach(d=>d.classList.remove("active")),t.classList.add("active"),f=s,l=1,v(f,l))}),o.addEventListener("click",r=>{const t=r.target;if(t.tagName.toLowerCase()!=="li")return;const s=Number(t.dataset.page);!s||s===l||(l=s,v(f,l))}),v(f,l,!1)}O();R();B();const q=document.querySelector(".exercises-content");q instanceof HTMLElement&&D(q);
