...
} from '../html-gererators/filter-html-generator';
...
export function initFilters() {
  ...
  const exercisesPaginationContainer = document.querySelector('.pagination');
  ...
  let currentFilter = (tabsContainer.querySelector('.active') as HTMLElement)
    ?.dataset.filter as FilterType;
  let currentPage = 1;
  ...
  function loadFilters(filter: FilterType, page: number) {
    ...
    getFilters({ filter, page, limit })
      .then(response => {
        filtersOutputContainer!.innerHTML = generateFiltersHtml(
          response.results
        );
  ...
  tabsContainer.addEventListener('click', event => {
    ...
    if (exercisesListContainer) exercisesListContainer.innerHTML = '';
    if (exercisesPaginationContainer)
      exercisesPaginationContainer.innerHTML = '';
  });
